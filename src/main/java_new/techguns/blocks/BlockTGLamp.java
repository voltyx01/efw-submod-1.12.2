...
public class BlockTGLamp<T extends Enum<T> & IStringSerializable> extends GenericBlock implements ICamoChangeable {
	...
	public IBlockState getStateFromMeta(int meta) {
		...
		if (meta <6) {
			s = s.withProperty(LAMP_TYPE, this.clazz.getEnumConstants()[0]).withProperty(FACING_ALL,EnumFacing.values()[meta]);
		} else if (meta <12) {
			s = s.withProperty(LAMP_TYPE, this.clazz.getEnumConstants()[1]).withProperty(FACING_ALL,EnumFacing.values()[meta-6]);
		} else if (meta == 12) {
	...
	public void registerItemBlockModels() {
		for(int i = 0; i< clazz.getEnumConstants().length;i++) {
			...
			ModelLoader.setCustomModelResourceLocation(itemblock, this.getMetaFromState(state), new ModelResourceLocation(new ResourceLocation("mwccf","lamp_inventory_"+clazz.getEnumConstants()[i].getName()),"inventory"));
		}
	}
	...
	public String getCurrentCamoName(ItemStack item) {
		return "mwccf"+"."+"tglamp"+".camoname."+this.getCurrentCamoIndex(item);
	}
