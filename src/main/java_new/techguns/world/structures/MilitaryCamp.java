...
//		structures.add(new FactoryHouseSmall(9, 6, 10, 16, 6, 16).setSwapXZ(true));
	...
	private void initCampFlagEntity(World world) {
		...
		world.setBlockState(p, TGBlocks.MONSTER_SPAWNER.getDefaultState().withProperty(((techguns.blocks.BlockTGSpawner)TGBlocks.MONSTER_SPAWNER).TYPE,EnumMonsterSpawnerType.SOLDIER_SPAWN),3);
		...
		for (int i=0;i<this.spawnPositions.size();i+=3) {
			...
			world.setBlockState(p.setPos(x,y,z), TGBlocks.MONSTER_SPAWNER.getDefaultState().withProperty(((techguns.blocks.BlockTGSpawner)TGBlocks.MONSTER_SPAWNER).TYPE,EnumMonsterSpawnerType.SOLDIER_SPAWN),3);
