...
import com.teamderpy.shouldersurfing.asm.transformers.EntityTurn;
...
public class ShoulderTransformer implements IClassTransformer
{
	...
	{
		String key = TRANSFORMERS.containsKey(name) ? name : (TRANSFORMERS.containsKey(transformedName) ? transformedName : null);
		if(key != null && bytes != null)
		{
			List<IShoulderTransformer> transformers = TRANSFORMERS.get(key);
	...
	{
		Map<String, List<IShoulderTransformer>> map = new java.util.HashMap<>();
		for (IShoulderTransformer transformer : transformers) {
			String obfName = MAPPINGS.map(transformer.getClassId(), true).replace('/', '.');
			String deobfName = MAPPINGS.map(transformer.getClassId(), false).replace('/', '.');
			map.computeIfAbsent(obfName, k -> new java.util.ArrayList<>()).add(transformer);
			map.computeIfAbsent(deobfName, k -> new java.util.ArrayList<>()).add(transformer);
		}
		return map;
	}
}
