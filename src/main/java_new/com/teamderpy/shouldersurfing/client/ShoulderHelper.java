...
public class ShoulderHelper
{
	...
	public static ShoulderLook shoulderSurfingLook(Entity entity, float partialTick, double distanceSq) {
		...
		boolean isDecoupled = ShoulderInstance.getInstance().doShoulderSurfing();

		float yaw   = isDecoupled ? renderer.cameraYaw   : entity.rotationYaw;
		float pitch = isDecoupled ? renderer.cameraPitch : entity.rotationPitch;
		...
		if (isDecoupled) {
			...
			// Поскольку cameraYaw смещен на 180 относительно взгляда игрока (под Forge onCameraSetup)
			...
			viewVector = entity.getLook(partialTick);
		}

		Vec3d cameraOffset = ShoulderHelper.calcCameraOffset(
				renderer.getCameraDistance(), yaw, pitch, partialTick
		);
		Vec3d eyePosition = entity.getPositionEyes(partialTick);
		Vec3d cameraPos   = eyePosition.add(cameraOffset);

		// headOffset = проекция cameraOffset на плоскость, перпендикулярную viewVector
		Vec3d headOffset = ShoulderHelper.calcPlaneWithLineIntersection(
				Vec3d.ZERO, viewVector, cameraOffset, viewVector
		);

		double extraDist = cameraOffset.distanceTo(headOffset);
		if (Config.CLIENT.limitPlayerReach()) {
			double len = headOffset.length();
			distanceSq = Math.max(0, distanceSq - len * len);
		}

		Vec3d traceEnd = eyePosition.add(headOffset)
				.add(viewVector.scale(Math.sqrt(distanceSq) + extraDist));
	...
	public static Vec3d calcCameraOffset(double distance, float yaw, float pitch, float partialTick)
	{
		...
		float finalYaw   = instance.doShoulderSurfing() ? renderer.cameraYaw   : yaw;
		float finalPitch = instance.doShoulderSurfing() ? renderer.cameraPitch : pitch;
		...
		// Масштабируем offset в зависимости от реальной дистанции до стены
		double maxDistance = Math.sqrt(offsetX * offsetX + offsetY * offsetY + offsetZ * offsetZ);
		double scale = maxDistance > 0 ? distance / maxDistance : 1.0;
		offsetX *= scale;
		offsetY *= scale;
		offsetZ *= scale;

		float yawRad   = finalYaw * DEG_TO_RAD;
		float pitchRad = finalPitch * DEG_TO_RAD;
		...
		Vec3d left    = new Vec3d(-cosYaw, 0, -sinYaw);

		// offsetX: влево (+), offsetY: вверх (+), offsetZ: назад (против вектора forward)
		return left.scale(offsetX)
	...
	public static RayTraceResult traceEntities(Entity cameraEntity, double playerReach, float partialTick, boolean shoulderSurfing)
	{
		...
		Vec3d viewVector = cameraEntity.getLook(1.0F).scale(playerReach);
		...
		if(shoulderSurfing)
		{
			...
			from = look.cameraPos(); // ФИКС: Исходим из камеры!
			to = look.traceEndPos();
			// Смещаем зону поиска сущностей к камере, чтобы она находила тех, кто перед экраном
			Vec3d cameraOffset = look.cameraPos().subtract(eyePosition);
			aabb = aabb.offset(cameraOffset);
		}
		else
		{
			from = eyePosition;
	...
	public static RayTraceResult traceBlocks(Entity entity, boolean stopOnFluid, double distance, float partialTick, boolean shoulderSurfing)
	{
		...
		if(shoulderSurfing)
		{
			...
			Vec3d from = look.cameraPos(); // ФИКС: Исходим из камеры!
