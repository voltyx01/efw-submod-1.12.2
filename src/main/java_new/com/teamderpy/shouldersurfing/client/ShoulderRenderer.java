...
public class ShoulderRenderer
{
	...
	private float getShadersResmul()
	{
		try
		{
			if(this.shaders == EnumShaderCompatibility.NEW)
			{
				Class<?> clazz = Class.forName("net.optifine.shaders.Shaders");
				if(clazz.getField("shaderPackLoaded").getBoolean(null))
				{
					return clazz.getField("configRenderResMul").getFloat(null);
				}
			}
			else if(this.shaders == EnumShaderCompatibility.OLD)
			{
