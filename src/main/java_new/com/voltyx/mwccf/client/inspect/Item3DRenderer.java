...
public class Item3DRenderer {
    ...
    private static final ResourceLocation BACKPACK_TEX = new ResourceLocation("quark",
            "textures/misc/backpack_worn.png");
    private static final ResourceLocation BACKPACK_OVERLAY_TEX = new ResourceLocation("quark",
            "textures/misc/backpack_worn_overlay.png");
    ...
    public static void renderConfigured3D(ItemStack stack, ItemInspectConfig.GroupTransform cfg, Minecraft mc) {
        ...
        // ИСПРАВЛЕНИЕ: Сначала включаем depthMask, ИНАЧЕ clear НЕ СРАБОТАЕТ!
        GlStateManager.enableDepth();
        GlStateManager.depthMask(true);
        GlStateManager.clear(256); // GL11.GL_DEPTH_BUFFER_BIT
        GlStateManager.depthFunc(515);

        // 2. ОТКЛЮЧАЕМ ОТСЕЧЕНИЕ ГРАНЕЙ
        ...
        // 3. ВЫДВИГАЕМ МОДЕЛЬ ВПЕРЕД К КАМЕРЕ (чтобы не пересекалась с фоном)
        ...
        GlStateManager.rotate(-cfg.startYaw, 180.0F, 1.0F, 0.0F);
        ...
        // Включаем Culling обратно, чтобы не сломать рендер остальной игры
    ...
    public static void render3D(ItemStack stack, float yaw, float pitch, float scale, Minecraft mc) {
        ...
        // ИСПРАВЛЕНИЕ: Включаем запись глубины ДО очистки!
        GlStateManager.enableDepth();
        GlStateManager.depthMask(true);
        ...
        // Выдвигаем вперед по Z
        // Отодвигаем вглубь, чтобы избежать обрезки передней части
    ...
    private static boolean isWeapon(ItemStack stack) {
        ...
        return stack.getItem() instanceof com.paneedah.weaponlib.Weapon || cls.contains("weaponlib")
                || cls.contains("Weapon");
    }
    ...
    private static void renderCustomArmor(ItemCustomArmor customArmor, ItemStack stack, Minecraft mc) {
        ...
        if (model != null) {
            ...
            // Render the specific slot parts directly so setRotationAngles / whole biped
            // aren't triggered
            float scale = 0.0625F;
            if (slot == EntityEquipmentSlot.HEAD) {
                if (model.bipedHead != null)
    ...
    private static void renderGenericItem(ItemStack stack, Minecraft mc) {
        ...
        // Only scale down Z thickness for NoteItem to look like a thin sheet of paper,
        // while preserving natural 3D dimensions for all blocks, tools, and other
        // items.
