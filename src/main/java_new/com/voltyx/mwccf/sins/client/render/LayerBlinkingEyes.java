...
public class LayerBlinkingEyes implements LayerRenderer<AbstractClientPlayer> {
    ...
    private void drawEyelid(float eyeX, float eyeY, float eyeW, float eyeH,
        ...
        buffer.pos(x2, y1, 0).tex(maxU, minV).endVertex();
        buffer.pos(x1, y1, 0).tex(minU, minV).endVertex();
        buffer.pos(x1, y2, 0).tex(minU, maxV).endVertex();
        buffer.pos(x2, y2, 0).tex(maxU, maxV).endVertex();
        tessellator.draw();
    }
