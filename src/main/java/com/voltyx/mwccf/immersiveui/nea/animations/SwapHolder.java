...
public class SwapHolder {
    ...
    public boolean init(Slot hoveredSlot, List<Slot> slots, int hotbarIndex) {
        if (ImmersiveUIConfig.isBlacklisted(Minecraft.getMinecraft().currentScreen) || ImmersiveUIConfig.moveAnimationTime <= 0) return false;
    ...
    public static Slot findHotbarSlot(List<Slot> slots, int index) {
        for (Slot slot : slots) {
            ...
            if (slot.inventory instanceof InventoryPlayer || (slot instanceof SlotItemHandler && (((SlotItemHandler) slot).getItemHandler() instanceof PlayerMainInvWrapper || ((SlotItemHandler) slot).getItemHandler() instanceof PlayerInvWrapper))) {
                return slot;
            }
        }
        return null;
