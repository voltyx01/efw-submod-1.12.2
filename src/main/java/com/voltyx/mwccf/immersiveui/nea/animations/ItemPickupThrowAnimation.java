...
public class ItemPickupThrowAnimation {
    ...
    private static final Map<IItemLocation, Long> animated = new HashMap<>();
    ...
    public static void animate(IItemLocation slot) {
        if (lastGui == null) return;
        animated.put(slot, NEAHelper.time());
    ...
    public static float getValue(GuiContainer container, IItemLocation slot) {
        if (lastGui != container || !animated.containsKey(slot)) return 1.0F;
        long time = animated.get(slot);
        float val = (NEAHelper.time() - time) / (float) ImmersiveUIConfig.appearAnimationTime;
        if (val >= 1.0F) {
            animated.remove(slot);
            return 1.0F;
        }
        return ImmersiveUIConfig.appearAnimationCurve.interpolate(0.0F, 1.0F, val);
    }

    public static void drawIndependentAnimations(GuiContainer container, RenderItem itemRender, FontRenderer fontRenderer) {
        for (int i = 0; i < removalAnimation.size(); i++) {
            IItemLocation slot = removalAnimation.get(i);
            int x = slot.nea$getX();
            int y = slot.nea$getY();
            float value = 1.0F - getValue(container, slot);
            if (value <= 0.0F) {
