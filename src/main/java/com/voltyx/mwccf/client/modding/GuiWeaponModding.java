...
public class GuiWeaponModding extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        // Динамически обновляем целевую позицию камеры (плавный отклик на глушитель /
        // смену категорий)
        updateWeaponCenter(false);
    ...
    private void renderWeapon3D(int mouseX, int mouseY) {
        ...
        ScaledResolution sr = new ScaledResolution(mc);
        int scaleFactor = Math.max(1, sr.getScaleFactor());
        // Reference is Fullscreen AUTO (scaleFactor = 6) so the model size is 1.5x
        // larger and stays constant regardless of GUI scale option
        float guiScaleRatio = 6.0f / (float) scaleFactor;

        // Base center of screen with screen-space camera pan
        float centerX = this.width / 2.0f + currentCamX * guiScaleRatio;
        float centerY = (this.height / 2.0f - 15.0f) + currentCamY * guiScaleRatio;

        GlStateManager.translate(centerX, centerY, 200.0f);

        float baseScale = WeaponModdingCenterHelper.getBaseScale(pwi) * currentZoom * guiScaleRatio * com.voltyx.mwccf.client.inspect.ItemInspectConfig.getGlobalCustomizationWeaponScale();
        GlStateManager.scale(baseScale, -baseScale, baseScale);

        // Smooth user interactive rotation (if held RMB)
        GlStateManager.rotate(currentPitch, 1.0F, 0.0F, 0.0F);
        GlStateManager.rotate(currentYaw, 0.0F, 1.0F, 0.0F);

        // Side-on flat profile: rotate +90 degrees around Y axis so weapon barrel
        // points to the LEFT
        GlStateManager.rotate(90.0F, 0.0F, 1.0F, 0.0F);

        // 1. Отрисовываем оружие со всеми деталями (оставляем твой оригинальный код)
        com.paneedah.weaponlib.animation.AnimationModeProcessor.getInstance().setActiveCategory(null);
        RenderHelper.enableStandardItemLighting();
        OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240.0F, 240.0F);
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        ...
        // ВАЖНО: Удален блок glUseProgram(0), ломавший шейдеры.
        ...
        if (activeCategory != null && pwi.getAttachmentItemWithCategory(activeCategory) != null) {
            ...
            try {
                ...
                try {
                    ...
                    // ГАРАНТИЯ: цвет всегда возвращается, даже если рендер VMW упадет
                ...
                // ПАСС C удален. Модуль УЖЕ был отрисован в шаге 1, нет нужды плодить
                // Z-fighting.
    ...
    private void renderWeaponStatsHeader() {
        ...
        renderStatBar(statsX, statsY + 20, isRussian() ? "ОТДАЧА КАМЕРЫ" : "CAMERA RECOIL", String.format("%.1f", recoil),
                Math.min(1.0f, recoil / 15.0f), 0xFFA040);
        renderStatBar(statsX, statsY + 30, isRussian() ? "ТОЧНОСТЬ В ПРИЦЕЛЕ" : "AIM ACCURACY",
                String.format("%.1f", Math.max(0.0f, 10.0f - (pwi.getInaccuracy() * (float) effStats.aimSpreadMultiplier))),
    ...
    private void renderAttachmentDropUpMenu(int mouseX, int mouseY, float deltaSec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (isHovered) {
                ...
                com.paneedah.weaponlib.stats.AttachmentStatData stats = com.paneedah.weaponlib.stats.AttachmentStatsManager.getStats(attachment);
                if (stats != null) {
                    boolean isRu = isRussian();
                    if (Math.abs(stats.recoilMultiplier - 1.0) > 0.001) {
                        double pct = (1.0 - stats.recoilMultiplier) * 100.0;
                        tooltipLines.add(pct > 0 ? (TextFormatting.GREEN + " ▲ " + (isRu ? "Контроль отдачи: +" : "Recoil: -") + String.format("%.0f%%", pct))
                                                 : (TextFormatting.RED + " ▼ " + (isRu ? "Отдача: +" : "Recoil: +") + String.format("%.0f%%", -pct)));
                    }
                    if (Math.abs(stats.visualRecoilMultiplier - 1.0) > 0.001) {
                        double pct = (1.0 - stats.visualRecoilMultiplier) * 100.0;
                        tooltipLines.add(pct > 0 ? (TextFormatting.GREEN + " ▲ " + (isRu ? "Стабилизация в руках: +" : "Stabilization: +") + String.format("%.0f%%", pct))
                                                 : (TextFormatting.RED + " ▼ " + (isRu ? "Смещение в руках: +" : "Kick: +") + String.format("%.0f%%", -pct)));
                    }
                    if (Math.abs(stats.hipSpreadMultiplier - 1.0) > 0.001) {
                        double pct = (1.0 - stats.hipSpreadMultiplier) * 100.0;
                        tooltipLines.add(pct > 0 ? (TextFormatting.GREEN + " ▲ " + (isRu ? "Точность от бедра: +" : "Hip Accuracy: +") + String.format("%.0f%%", pct))
                                                 : (TextFormatting.RED + " ▼ " + (isRu ? "Разброс от бедра: +" : "Hip Spread: +") + String.format("%.0f%%", -pct)));
                    }
                    if (Math.abs(stats.adsSpeedMultiplier - 1.0) > 0.001) {
                        double pct = (stats.adsSpeedMultiplier - 1.0) * 100.0;
                        tooltipLines.add(pct > 0 ? (TextFormatting.GREEN + " ▲ " + (isRu ? "Скорость вскидки: +" : "ADS Speed: +") + String.format("%.0f%%", pct))
                                                 : (TextFormatting.RED + " ▼ " + (isRu ? "Скорость вскидки: " : "ADS Speed: ") + String.format("%.0f%%", pct)));
                    }
                    if (Math.abs(stats.drawSpeedMultiplier - 1.0) > 0.001) {
                        double pct = (stats.drawSpeedMultiplier - 1.0) * 100.0;
                        tooltipLines.add(pct > 0 ? (TextFormatting.GREEN + " ▲ " + (isRu ? "Скорость доставания: +" : "Draw Speed: +") + String.format("%.0f%%", pct))
                                                 : (TextFormatting.RED + " ▼ " + (isRu ? "Скорость доставания: " : "Draw Speed: ") + String.format("%.0f%%", pct)));
                    }
                    if (Math.abs(stats.reloadSpeedMultiplier - 1.0) > 0.001) {
                        double pct = (stats.reloadSpeedMultiplier - 1.0) * 100.0;
                        tooltipLines.add(pct > 0 ? (TextFormatting.GREEN + " ▲ " + (isRu ? "Скорость перезарядки: +" : "Reload Speed: +") + String.format("%.0f%%", pct))
                                                 : (TextFormatting.RED + " ▼ " + (isRu ? "Скорость перезарядки: " : "Reload Speed: ") + String.format("%.0f%%", pct)));
                    }
                    if (Math.abs(stats.weight) > 0.001) {
                        tooltipLines.add(stats.weight > 0 ? (TextFormatting.RED + " ▼ " + (isRu ? "Вес: +" : "Weight: +") + String.format("%.2f кг", stats.weight))
                                                          : (TextFormatting.GREEN + " ▲ " + (isRu ? "Вес: " : "Weight: ") + String.format("%.2f кг", stats.weight)));
                    }
                }
            }
        }
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        if (keyCode == Keyboard.KEY_ESCAPE || keyCode == Keyboard.KEY_X || keyCode == Keyboard.KEY_E
                || (mc.gameSettings.keyBindInventory != null && keyCode == mc.gameSettings.keyBindInventory.getKeyCode())) {
            if (activeCategory != null) {
                activeCategory = null;
                updateWeaponCenter(true);
                return;
            }
            GuiScreen target = (this.parentScreen != null) ? this.parentScreen : new net.minecraft.client.gui.inventory.GuiInventory(this.mc.player);
