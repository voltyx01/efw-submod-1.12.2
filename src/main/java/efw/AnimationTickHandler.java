...
public class AnimationTickHandler {
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (weaponType != efw.animation.WeaponTypeHelper.WeaponType.NONE && animName != null
            ...
            try {
                ...
                if (weaponInstance != null) {
                    ...
                    if (state != null) {
                        ...
                        // Safeguard for all reloads (standing & lying): if logical reload has ended
                        // (weapon is no longer in a reload/load/unload state), immediately cut off
                        // the reload animation with a smooth blend so logic and visuals finish synchronously!
                        if (currentlyReloading && !isReloadState) {
                            currentlyReloading = false;
                        }
            ...
            // Calculate effective reload speed multiplier from attachments & config
            ...
            try {
                com.paneedah.weaponlib.PlayerWeaponInstance weaponInstance = (com.paneedah.weaponlib.PlayerWeaponInstance)
                    com.paneedah.weaponlib.ClientModContext.getContext()
                        ...
                        .getMainHandItemInstance(player, com.paneedah.weaponlib.PlayerWeaponInstance.class);
                if (weaponInstance != null) {
                    double mult = com.paneedah.weaponlib.config.ModernConfigManager.reloadSpeedMultiplier;
                    ...
                    if (effStats != null) {
                        mult *= effStats.reloadSpeedMultiplier;
                    }
                    if (mult > 0.01) {
                        reloadSpeedMult = (float) mult;
                    }
                }
            } catch (Throwable t) {}
                ...
                if (isReloading) {
                    ...
                    if (reloadClip != null && !isRolling) {
                        if (!ap.isActionPlaying() || !actionCandidate.equals(ap.getCurrentActionName())) {
                            // Та же скорость и та же логика бленда (getActionBlendTicks: "lie_reload" -> 10),
                            // что и у reload_upper стоя — симметрично, масштабируется с обвесами.
                            ap.setAction(reloadClip, 0f, 0.83f * reloadSpeedMult);
                        } else {
                            ap.resumeActionLoop();
                        }
                    }
                } else if (!"roll".equals(ap.getCurrentActionName())) {
                    // ВАЖНО: вызывать безусловно (без доп. проверки isActionPlaying()!).
                    // Когда lie_reload доигрывает до конца САМ (не через обрезание/cut),
                    // isActionPlaying() уже false (клип не зациклен), но actionLayer
                    // всё ещё хранит "замороженный" на последнем кадре fade-модификатор,
                    // и get3DTransform() применяет его БЕЗУСЛОВНО (проверяет != null, а не isActive()).
                    // Если не вызвать cancelAction() здесь, рука навсегда останется в последнем
                    // кадре reload ("съезжает"/залипает). cancelAction()/stopAction() сам по себе
                    // дешёвый и безопасный при повторных вызовах (есть внутренние guard'ы).
                    ap.cancelAction();
                }
        ...
        // ВАЖНО: раньше здесь проверялось animName (переприсвоенное имя БАЗОВОЙ позы —
        // "lie"/"lie_move"/"rifle_lie_aim" и т.д.), которое НИКОГДА не содержит "reload"/"fire" —
        // это имя живёт только в клипе ЭКШЕН-слоя ("rifle_lie_reload"). Из-за этого isLyingAction
        // был всегда false во время лежачей перезарядки, и код замораживал базовую позу
        // (baseSpeed = 0f при !isMoving), пока рука на экшен-слое продолжала крутить reload
        // с фиксированной скоростью — рассинхрон, из-за которого лежачая перезарядка выглядела
        // и ощущалась иначе, чем стоячая. Проверяем реальное имя текущего экшена вместо базовой позы.
        String currentActionNameForSpeed = ap.getCurrentActionName();
        boolean isLyingAction = currentActionNameForSpeed != null
                && (currentActionNameForSpeed.contains("reload") || currentActionNameForSpeed.contains("fire"));
