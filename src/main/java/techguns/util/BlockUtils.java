...
public class BlockUtils {
	...
	protected static boolean checkMiningLevels(World world, EntityPlayer ply, BlockPos b, Object miningtool, ItemStack stack) {
		...
		if (state.getBlock().canCollideCheck(state, false) && state.getBlock().canHarvestBlock(world, b, ply)) {
			if(tool==null) {
				...
				return state.getBlock().getHarvestLevel(state) <= ((net.minecraft.item.Item)miningtool).getHarvestLevel(stack, tool, ply, state);
			}
		}
		return false;
	}
	
	public static List<BlockPos> getBlockPlaneAroundAxisForMining(World world, EntityPlayer ply, BlockPos center, EnumFacing.Axis axis, int radius, boolean includeCenter, @Nullable Object miningtool, ItemStack stack){
