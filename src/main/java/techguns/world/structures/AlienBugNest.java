...
public class AlienBugNest {
	...
	static {
		wallBlock = new MultiMBlock(new Block[]{net.minecraft.init.Blocks.AIR, net.minecraft.init.Blocks.AIR, Blocks.SANDSTONE}, new int[]{EnumTGSandHardTypes.BUGNEST_SAND.ordinal(),EnumTGSlimyType.BUGNEST_EGGS.ordinal(),0}, new int[]{16,1,2});
		wallBlock2 = new MultiMBlock(new Block[]{net.minecraft.init.Blocks.AIR, Blocks.SANDSTONE}, new int[]{EnumTGSandHardTypes.BUGNEST_SAND.ordinal(),0}, new int[]{12,1});
		innerBlock = new MBlock(Blocks.AIR,0);
		//slimeLadder = new MBlock(net.minecraft.init.Blocks.AIR,0);//new MBlock(TGBlocks.bugnestslimy, 0);
	    slimyBlock = new MBlockSlime(net.minecraft.init.Blocks.AIR, EnumTGSandHardTypes.BUGNEST_SAND.ordinal());
	    spawner = new MBlockTGSpawner(EnumMonsterSpawnerType.HOLE, 7, 3, 150, 1);
	    eggsBlock = new MBlock(net.minecraft.init.Blocks.AIR, EnumTGSlimyType.BUGNEST_EGGS.ordinal());
...
//	}
		...
		for (int x = x1-r; x < x2+r; x++) {
			for (int y = y1-r; y < y2+r; y++) {
				for (int z = z1-r; z < z2+r; z++) {
					...
					p.setPos(x,y,z);
					if (distance < radius  && net.minecraft.init.Blocks.AIR.canPlaceBlockAt(world, p)) {
						world.setBlockState(p, net.minecraft.init.Blocks.AIR.getStateForPlacement(world, p, EnumFacing.NORTH, 0, 0, 0, 0, null), 2);
	...
	static class MBlockSlime extends MBlock {
		...
		private void placeSlimeAt(World world, MutableBlockPos pos) {
			if (world.isAirBlock(pos) && net.minecraft.init.Blocks.AIR.canPlaceBlockAt(world, pos)) { //TGBlocks.bugnestslimy.canPlaceBlockAt(world, x, y, z)) {						
				//slimeLadder.setBlock(world, pos, 0, null);
				world.setBlockState(pos, net.minecraft.init.Blocks.AIR.getStateForPlacement(world, pos, EnumFacing.NORTH, 0, 0, 0, 0, null), 2);
				//world.setBlockMetadataWithNotify(x,y,z,((BlockBugSlime)TGBlocks.bugnestslimy).getDirectionMeta(world, x, y, z), 2);
			}
		}
