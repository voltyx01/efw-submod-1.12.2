...
public class WeaponRenderer extends ModelSource implements IBakedModel {
	...
	private static float lowerProgress = 0f; // [0..1], 0=normal, 1=fully lowered
	private static float lowerProgressPrev = 0f;
	private static long lowerStartTime = -1L;
	static final long LOWER_DURATION_MS = 175L;
	...
	/**
	 * Called when the player switches away from a hotbar slot. Main client thread
	 * only.
	 */
	public static void triggerLowering(ItemStack outgoingStack, int fromSlot) {
		lowerProgress = 0f;
	...
	public static void handleHotbarSlotChange(EntityPlayer player) {
		...
		if (incomingIsWeapon) {
			// Always restart the lowering animation, even if it was already active
			triggerLowering(outgoingStack, oldSlot);
		} else {
			if (isLoweringActive) {
	...
	public static void updateLoweringProgress() {
		...
		if (lowerStartTime >= 0L) {
			...
			lowerProgress = Math.min(1f, elapsed / (float) LOWER_DURATION_MS);
			if (lowerProgress >= 1f) {
				lowerStartTime = -1L;
				isLoweringActive = false;
				loweringItemStack = null;
	...
	public static class Builder {
		...
		public Builder setupCompoundReload(String animationFile, String anim, String mainBoneName,
			...
			this.compoundReloadContainer
					.setLeftHand(left.getTransitionList(firstPersonLeftHandTransform, BBLoader.HANDDIVISOR));
			this.compoundReloadContainer.setRightHand(right == null ? null
					: right.getTransitionList(firstPersonRightHandTransform, BBLoader.HANDDIVISOR));
	...
	private void executeWithSlideLockSuppression(Consumer<RenderContext<RenderableState>> weaponPositionFunction,
			RenderContext<RenderableState> context) {
		PlayerWeaponInstance pwi = context.getWeaponInstance();
		if (pwi != null) {
			WeaponState state = pwi.getState();
			boolean reloading = state == WeaponState.COMPOUND_RELOAD_EMPTY ||
					state == WeaponState.COMPOUND_RELOAD_FINISH ||
					state == WeaponState.COMPOUND_RELOAD_FINISHED;
			boolean justFinishedReload = state == WeaponState.READY &&
					pwi.isMagSwapDone() &&
					(System.currentTimeMillis() - pwi.getStateUpdateTimestamp() < 2000);
			if (reloading || justFinishedReload) {
				pwi.setSuppressSlideLock(true);
				try {
					weaponPositionFunction.accept(context);
				} finally {
					pwi.setSuppressSlideLock(false);
				}
				return;
			}
	...
	private Consumer<RenderContext<RenderableState>> createWeaponPartPositionFunction(
		...
		if (weaponPositionFunction != null) {
			return context -> executeWithSlideLockSuppression(weaponPositionFunction, context);
		}
	...
	private List<MultipartTransition<Part, RenderContext<RenderableState>>> makeThirdPersonStatic(
		...
		for (MultipartTransition<Part, RenderContext<RenderableState>> original : transitions) {
			...
			t.withPartPositionFunction(Part.MAIN_ITEM,
					createWeaponPartPositionFunction(getBuilder().thirdPersonPositioning));
			t.withPartPositionFunction(Part.LEFT_HAND, createWeaponPartPositionFunction(context -> {
			}));
			t.withPartPositionFunction(Part.RIGHT_HAND, createWeaponPartPositionFunction(context -> {
			}));
	...
	public void renderItem(ItemStack weaponItemStack, RenderContext<RenderableState> renderContext,
		...
		if (!OpenGLSelectionHelper.isInSelectionPass && !AnimationGUI.getInstance().magEdit.isState()) {
			...
			if (ModernConfigManager.enableAllShaders && ModernConfigManager.enableGunShaders) {
				...
				GL20.glUniform1f(GL20.glGetUniformLocation(Shaders.gunLightingShader.getShaderId(), "lightIntensity"),
	...
	private void renderCompatibleAttachment(CompatibleAttachment<?> compatibleAttachment,
		...
		if (compatibleAttachment.getAttachment().getCategory() == AttachmentCategory.MAGICMAG) {
			...
			// The *1.2 is important so that the magazine is there until the animation is
			// fully completed
			...
			// ItemAttachment<Weapon> exp =
			// renderContext.getModContext().getAttachmentAspect().getActiveAttachment(renderContext.getWeaponInstance(),
		...
		if (compatibleAttachment.getAttachment() instanceof ItemMagazine && AnimationGUI.getInstance().magEdit.isState()
			...
			// GlStateManager.translate(mag.getRotationPoint().x, mag.getRotationPoint().y,
			// mag.getRotationPoint().z);
		...
		// double distanceSq = this.player != null ?
		// renderViewEntity.getDistanceSq(this.player) : 0;
		...
		for (Tuple<ModelBase, String> texturedModel : compatibleAttachment.getAttachment().getTexturedModels()) {
			...
			if (compatibleAttachment.getModelPositioning() != null) {
				 ...
				 * compatibleAttachment.getModelPositioning().accept(texturedModel.getU());
				 * } else {
				 * new Transform().withScale(1, 1,
				 * 1).withRotationPoint(ClientEventHandler.magRotPositioner.x,
				 * ClientEventHandler.magRotPositioner.y,
				 * ClientEventHandler.magRotPositioner.z).withRotation(45, 0, 0).doGLDirect();
				 * }
				 */
				...
				/*
				 * if(compatibleAttachment.getAttachment().getCategory() ==
				 ...
				 * if(renderContext.getWeaponInstance().isSlideLocked() && (state ==
				 * WeaponState.READY || state == WeaponState.ALERT)) {
				 ...
				 * new Transform().withScale(1, 1,
				 * 1).withRotationPoint(ClientEventHandler.magRotPositioner.x,
				 * ClientEventHandler.magRotPositioner.y, ClientEventHandler.magRotPositioner.z)
				...
				if (possibleActionList != null && possibleActionList.length > 0) {
					for (ItemAttachment<Weapon> part : possibleActionList) {
						if (compatibleAttachment.getAttachment() == part) {
							...
							Transform t = renderContext.getWeaponInstance().getWeapon().getRenderer()
									.getBuilder().actionPieceTransform;
							...
							GlStateManager.translate(t.getPositionX() * mu, t.getPositionY() * mu,
									t.getPositionZ() * mu);
	...
	public void renderItem() {
		...
		if (transformType == ItemCameraTransforms.TransformType.FIRST_PERSON_RIGHT_HAND
			...
			if (weaponInstance != null && weaponInstance.getWeapon() != null) {
				isShotgun = weaponInstance.getWeapon()
						.getConfigurationGroup() == com.paneedah.weaponlib.config.BalancePackManager.GunConfigurationGroup.SHOTGUN;
			}
			float maxTilt = isShotgun ? 0f : 40f;
			float sneakTarget = (!isAiming && player instanceof EntityPlayer && player.isSneaking()) ? maxTilt : 0f;
			sneakTilt += (sneakTarget - sneakTilt) * 0.05f;
		}
		// ---------------------------------------------------------------------------------
		...
		switch (transformType) {
			...
			// PIVOT_X: negative = pivot moves right on screen (X is flipped by glScale)
			{
				...
				float lerpedTilt = sneakTiltPrev + (sneakTilt - sneakTiltPrev) * partialTicks;
