...
//        .withStepSound("step")
                ...
                .withPickupItemID(1)
                .withAiTargetTask(1, e -> new EntityAIHurtByTarget((EntityCreature) e, false))
                .withAiTargetTask(2, e -> new EntityAINearestAttackableTarget<>((EntityCreature) e, net.minecraft.entity.EntityLivingBase.class, 10, true, false, target -> {
                    if (target == null) return false;
                    if (target instanceof EntityCustomMob) {
                        EntityConfiguration cfg = ((EntityCustomMob) target).getConfiguration();
                        if (cfg != null && cfg.getName() != null && cfg.getName().startsWith("turret")) {
                            return false;
                        }
                    }
                    return target instanceof IMob;
                }))
                .withAiTargetTask(3, e -> new BetterAINearestAttackableTarget<>((EntityCreature) e, EntityCustomMob.class, "terrorist", true))
                .withAiTargetTask(3, e -> new BetterAINearestAttackableTarget<>((EntityCreature) e, EntityCustomMob.class, "tyke", true))
                .register(MWC.modContext);
        new EntityConfiguration.Builder()
