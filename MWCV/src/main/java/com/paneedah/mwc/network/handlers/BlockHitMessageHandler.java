...
public final class BlockHitMessageHandler implements IMessageHandler<BlockHitMessage, IMessage> {
    ...
    public IMessage onMessage(final BlockHitMessage blockHitMessage, final MessageContext messageContext) {
        NetworkUtil.processMessage(messageContext, () -> {
            ...
            com.paneedah.weaponlib.ClientEventHandler.BULLET_HOLE_RENDERER.addBulletHole(
                new com.paneedah.weaponlib.BulletHoleRenderer.BulletHole(
                    new io.redstudioragnarok.redcore.vectors.Vector3D(position.x, position.y, position.z),
                    blockHitMessage.getBlockPos(),
                    blockHitMessage.getEnumFacing(),
                    0.05
                )
            );
        });
