...
//        }
    ...
    // --- Оптимизация для Shoulder Surfing ---
    private static Method shoulderSurfingTurnMethod;
    private static boolean shoulderSurfingChecked = false;

    static {
        try {
            // �\ufffdщем класс и метод один раз при загрузке класса
            Class<?> delegator = Class.forName("com.teamderpy.shouldersurfing.asm.InjectionDelegation");
            shoulderSurfingTurnMethod = delegator.getMethod("Entity_turn", net.minecraft.entity.Entity.class, float.class, float.class);
        } catch (ClassNotFoundException | NoSuchMethodException e) {
            // Если мода нет, метод останется null, мы просто пропустим вызов
        }
    }
    ...
    public static void turn(EntityPlayer player, float yawDelta, float pitchDelta) {
        // 1. Оставляем логику отдачи и чувствительности прицелов MWC (чтобы ганплей не сломался)
        if(ClientModContext.getContext() != null && ClientModContext.getContext().getMainHeldWeapon() != null) {
            PlayerWeaponInstance pwi = ClientModContext.getContext().getMainHeldWeapon();
        ...
        // === 2. МАГ�\ufffdЯ СОВМЕСТ�\ufffdМОСТ�\ufffd С SHOULDER SURFING ===
        // Пробуем вызвать хук камеры через рефлексию (чтобы не добавлять мод в зависимости MWC).
        // Если ShoulderSurfing включен, он заберет дельты мыши себе и вернет true.
        if (shoulderSurfingTurnMethod != null) {
            try {
                boolean handled = (Boolean) shoulderSurfingTurnMethod.invoke(null, player, yawDelta, pitchDelta);
                if (handled) {
                    return; // Успешно передали управление, прерываем выполнение
                }
            } catch (Exception e) {
                // Если произошла ошибка при вызове (например, конфликт версий),
                // можно занулить метод, чтобы не пытаться вызвать его снова
                shoulderSurfingTurnMethod = null;
            }
        }
        // =================================================

        // 3. Стандартное вращение тела для MWC (если мод SS выключен или игрок в машине)
        float maxPitch = 90f;
        ...
        if(player.getRidingEntity() instanceof EntityVehicle && MC.gameSettings.thirdPersonView == 0) {
            ...
            if (player.rotationPitch < -maxPitch) player.rotationPitch = -maxPitch;
            if (player.rotationPitch > maxPitch) player.rotationPitch = maxPitch;
