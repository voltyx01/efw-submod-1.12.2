...
public class CustomGui extends Gui {
	...
	public final void onRenderHud(RenderGameOverlayEvent.Pre event) {
		handleShellPositionerHUD(event);
	...
	public void handleShellPositionerHUD(RenderGameOverlayEvent.Pre event) {
		if (event.getType() == RenderGameOverlayEvent.ElementType.ALL && com.paneedah.weaponlib.render.shells.ShellPositionManager.isAdjusterOpen) {
			EntityPlayer player = MC.player;
			PlayerWeaponInstance pwi = modContext.getMainHeldWeapon();
			com.paneedah.weaponlib.render.shells.ShellPositionManager.updateAutoState(player, pwi);
			...
			double step = isShift ? 0.01 : (isCtrl ? 0.10 : 0.05);
			...
			net.minecraft.client.gui.Gui.drawRect(x0, y0, x0 + w, y0 + h, 0xCC101015);
			net.minecraft.client.gui.Gui.drawRect(x0, y0, x0 + w, y0 + 1, 0xFF4CAF50);
			net.minecraft.client.gui.Gui.drawRect(x0, y0 + 16, x0 + w, y0 + 17, 0x55FFFFFF);

			font.drawStringWithShadow(TextFormatting.GREEN + "" + TextFormatting.BOLD + "MWC SHELL POSITION ADJUSTER", x0 + 8, y0 + 5, 0xFFFFFF);
			font.drawStringWithShadow(TextFormatting.GRAY + "Категория: " + TextFormatting.GOLD + cat.displayName + TextFormatting.DARK_GRAY + " [R]", x0 + 8, y0 + 22, 0xFFFFFF);
			font.drawStringWithShadow(TextFormatting.GRAY + "Режим: " + TextFormatting.AQUA + mode.displayName + TextFormatting.DARK_GRAY + " [TAB]", x0 + 8, y0 + 34, 0xFFFFFF);
			font.drawStringWithShadow(TextFormatting.GRAY + "Следовать: " + (com.paneedah.weaponlib.render.shells.ShellPositionManager.autoFollowState ? TextFormatting.GREEN + "ВКЛ" : TextFormatting.RED + "ВЫКЛ") + TextFormatting.DARK_GRAY + " [F]", x0 + 180, y0 + 34, 0xFFFFFF);

			String posStr = String.format(java.util.Locale.US, "X: %.3f   Y: %.3f   Z: %.3f", cur.x, cur.y, cur.z);
			font.drawStringWithShadow(TextFormatting.YELLOW + "" + TextFormatting.BOLD + posStr, x0 + 8, y0 + 48, 0xFFFFFF);
			font.drawStringWithShadow(TextFormatting.DARK_AQUA + String.format(java.util.Locale.US, "Шаг: %.2f (Shift=0.01, Ctrl=0.10)", step), x0 + 8, y0 + 62, 0xFFFFFF);
			font.drawStringWithShadow(TextFormatting.GRAY + "[←/→] X  [↑/↓] Y  [PgUp/PgDn] Z  [Enter] Сохр", x0 + 8, y0 + 78, 0xAAAAAA);
	...
	public void handleModificationHUD(RenderGameOverlayEvent.Pre event, PlayerWeaponInstance weaponInstance, double scaledWidth,
		...
		if (isInAltModifyingState(weaponInstance) || isInModifyingState(weaponInstance)) {
			// Old HUD overlay disabled in favor of new GuiWeaponModding
		} else {
	...
	public final void onRenderCrosshair(RenderGameOverlayEvent.Pre event) {
		...
		if(weaponInstance != null) {
			...
			if(crosshair != null) {
				// Cancel vanilla crosshair — MWC draws its own
				event.setCanceled(true);
				...
				// No custom crosshair texture defined — hide vanilla crosshair anyway for weapons
				event.setCanceled(true);
                handleModificationHUD(event, modContext.getMainHeldWeapon(), width, height);
                handleOpenDoorHUD(event, width, height);
			}
