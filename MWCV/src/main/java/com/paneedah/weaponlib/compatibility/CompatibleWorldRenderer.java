...
public class CompatibleWorldRenderer extends EntityRenderer
{
    ...
    public void updateRenderer()
    {
        ...
        if (MC.ingameGUI != null) {
            this.rendererUpdateCount = MC.ingameGUI.getUpdateCounter();
        }
        // Disabled: addRainParticles() causes duplicate particles and loud rain sounds every frame during scope zoom
    ...
    private void setupCameraTransform(float partialTicks, int pass)
    {
        ...
        // Sync zoomed matrices to RenderLib (used by Nothirium shaders & frustum culling)
        try {
            Class<?> renderUtilClass = Class.forName("meldexun.renderlib.util.RenderUtil");
            renderUtilClass.getMethod("update", double.class).invoke(null, (double)partialTicks);
...
//    }
    ...
    public void renderWorld(float partialTicks, long finishTimeNano)
    {
        ...
        // Disabled in scope: do not override main camera crosshair raycast
        ...
        // Restore clean OpenGL state so scope rendering doesn't leak into main scene
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        updateExternalRenderers(entity, partialTicks, icamera);
        ...
        // Disabled in scope: renderRainSnow() draws weather box right in front of high-zoom optical scopes causing twitching/frozen planes
        ...
        // Disabled in scope: dispatchRenderLast() fires RenderWorldLastEvent in secondary scope framebuffer, causing particlerain fog flickering & mod state corruption
...
//        }
    ...
    private void updateFogColor(float partialTicks)
    {
        ...
        // Disabled in scope: do not post EntityViewRenderEvent.FogColors in secondary scope pass to prevent external mods (ParticleRain) from flickering
    ...
    private void setupFog(int startCoords, float partialTicks)
    {
        ...
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        IBlockState iblockstate = ActiveRenderInfo.getBlockStateAtEntityViewpoint(MC.world, entity, partialTicks);
        float hook = -1.0F; // net.minecraftforge.client.ForgeHooksClient.getFogDensity(this, entity, iblockstate, partialTicks, 0.1F);
    ...
    private void updateExternalRenderers(Entity entity, float partialTicks, ICamera icamera) {
        try {
            // Update RenderLib camera state (matrices and frustum)
            Class<?> renderUtilClass = Class.forName("meldexun.renderlib.util.RenderUtil");
            renderUtilClass.getMethod("update", double.class).invoke(null, (double)partialTicks);
            renderUtilClass.getMethod("updateCamera").invoke(null);
        } catch (Throwable ignored) {}
        ...
        try {
            // Setup Nothirium ChunkRenderManager with scope camera frustum
            Class<?> chunkRenderManagerClass = Class.forName("meldexun.nothirium.mc.renderer.ChunkRenderManager");
            java.lang.reflect.Method setupMethod = chunkRenderManagerClass.getMethod("setup");
            setupMethod.invoke(null);
        } catch (Throwable ignored) {
            // Nothirium / RenderLib are not installed
        }
    }
}
