...
public class ClientEventHandler {
	...
	public static final java.util.Map<Integer, Long> lastShotTimeByEntity = new java.util.concurrent.ConcurrentHashMap<>();
	...
	private static final UUID SLOW_DOWN_WEAPON_WEIGHT_ATTRIBUTE_MODIFIER_UUID = UUID.fromString("c4e85291-7140-496a-b28f-1a98c527e5a1");
	...
	private static boolean hasPreloadedTextures = false;

	@SubscribeEvent
	@SideOnly(Side.CLIENT)
	public final void onTick(TickEvent.ClientTickEvent event) {
		if (!hasPreloadedTextures && MC.world != null) {
			hasPreloadedTextures = true;
			// Preload heavy textures to avoid a lag spike on the first shot
			MC.getTextureManager().bindTexture(com.paneedah.weaponlib.particle.BetterMuzzleSmoke.SMOKE1);
			MC.getTextureManager().bindTexture(com.paneedah.weaponlib.particle.BetterMuzzleSmoke.SMOKE2);
			MC.getTextureManager().bindTexture(new net.minecraft.util.ResourceLocation(com.paneedah.mwc.utils.ModReference.ID + ":textures/smokes/smoke4.png"));
		}

		final EntityPlayer player = MC.player;
		if (player != null) {
			WeaponRenderer.handleHotbarSlotChange(player);
		}
		...
		if (player != null && event.phase == TickEvent.Phase.END) {
			...
			final double xAmount = ClientValueRepo.recoilWoundX * 0.2;
			player.rotationYaw += xAmount;
			ClientValueRepo.recoilWoundX -= xAmount;
	...
	private void updateOnStartTick() {
	    if (MC.player != null) {
	        WeaponRenderer.updateLoweringProgress();
	    }
	}
	...
	public void onRenderSpecificHand(RenderSpecificHandEvent event) {
		if (event.getHand() != EnumHand.MAIN_HAND || MC.player == null || MC.gameSettings.thirdPersonView != 0) {
			return;
		}

		WeaponRenderer.updateLoweringProgress();
		WeaponRenderer.handleHotbarSlotChange(MC.player);

		if (WeaponRenderer.hasPendingDraw()) {
			if (!WeaponRenderer.isLoweringActive || WeaponRenderer.lowerProgress >= 1f) {
				net.minecraft.item.ItemStack currentStack = MC.player.getHeldItemMainhand();
				if (event.getItemStack() != null && currentStack != null && event.getItemStack().getItem() == currentStack.getItem()) {
					WeaponRenderer.executePendingDraw();
				}
			}
		}

		if (!WeaponRenderer.isLoweringActive) {
			return;
		}
		...
		if (MC.player instanceof AbstractClientPlayer) {
			WeaponRenderer.renderFirstPersonLoweringHand(
    ...
    private void update() {
		...
		if (mainHandHeldWeaponInstance != null) {
			...
			if (com.paneedah.weaponlib.config.ModernConfigManager.holdToAim && !mainHandHeldWeaponInstance.isAimed() && 
					(mainHandHeldWeaponInstance.getState() == WeaponState.READY || (mainHandHeldWeaponInstance.getState() == WeaponState.COMPOUND_RELOAD_FINISHED && mainHandHeldWeaponInstance.wasReloadedFromEmpty))) {
			    if (MC.gameSettings.keyBindUseItem.isKeyDown()) {
			        mainHandHeldWeaponInstance.setAimed(true);
			    }
			}
			...
			// Weapon weight movement penalty:
			// Full realistic weight: base weapon weight (~2.5 - 3.5kg) + attachment weight
			com.paneedah.weaponlib.stats.AttachmentStatsManager.EffectiveWeaponStats effStats =
					com.paneedah.weaponlib.stats.AttachmentStatsManager.getEffectiveStats(mainHandHeldWeaponInstance);
			double totalWeight = effStats.totalWeight;
			boolean isPistol = false;
			if (mainHandHeldWeaponInstance.getWeapon() != null) {
				String wName = mainHandHeldWeaponInstance.getWeapon().getTranslationKey().toLowerCase();
				isPistol = wName.contains("pistol") || wName.contains("glock") || wName.contains("m9") || wName.contains("makarov") || wName.contains("deagle") || wName.contains("revolver");
			}
			double baseGunWeight = isPistol ? 0.9 : 2.8;
			double netWeight = baseGunWeight + totalWeight;

			if (netWeight > 0.5) {
				// E.g. 4.0kg full-mod rifle -> -16% movement speed; 2.8kg base rifle -> -8% movement speed; pistol -> -3%
				double speedPenalty = -Math.min(0.40, Math.max(0.0, (netWeight - 1.0) * 0.05 + totalWeight * 0.05));
				AttributeModifier weightMod = new AttributeModifier(SLOW_DOWN_WEAPON_WEIGHT_ATTRIBUTE_MODIFIER_UUID, "Weapon Weight Slowdown", speedPenalty, 2).setSaved(false);
				PlayerUtil.restorePlayerSpeed(player, weightMod);
				PlayerUtil.slowPlayerDown(player, weightMod);
			} else {
				if (player.getEntityAttribute(net.minecraft.entity.SharedMonsterAttributes.MOVEMENT_SPEED).getModifier(SLOW_DOWN_WEAPON_WEIGHT_ATTRIBUTE_MODIFIER_UUID) != null) {
					player.getEntityAttribute(net.minecraft.entity.SharedMonsterAttributes.MOVEMENT_SPEED).removeModifier(SLOW_DOWN_WEAPON_WEIGHT_ATTRIBUTE_MODIFIER_UUID);
				}
			}
    ...
    public final void onRenderTickEvent(TickEvent.RenderTickEvent event) {
        com.paneedah.weaponlib.compatibility.CameraOverhaulCompat.updateSwayFade();
	...
	public void onRenderWorldLastEvent(RenderWorldLastEvent event) {
		...
		if (com.paneedah.weaponlib.render.shells.ShellPositionManager.isAdjusterOpen && MC.player != null && MC.world != null) {
			EntityPlayer p = MC.player;
			com.paneedah.weaponlib.render.shells.ShellPositionManager.WeaponCategory cat = com.paneedah.weaponlib.render.shells.ShellPositionManager.selectedCategory;
			com.paneedah.weaponlib.render.shells.ShellPositionManager.ViewMode mode = com.paneedah.weaponlib.render.shells.ShellPositionManager.selectedMode;
			Vec3d cur = com.paneedah.weaponlib.render.shells.ShellPositionManager.getOffset(cat, mode);
			...
			Vec3d weaponDir = new Vec3d(cur.x, cur.y, cur.z + fovExtra).rotatePitch((float) Math.toRadians(-p.rotationPitch)).rotateYaw((float) Math.toRadians(-p.rotationYaw));
			Vec3d spawnPos = p.getPositionEyes(1.0f).add(weaponDir);

			com.paneedah.weaponlib.render.shells.ShellParticleSimulator.Shell.Type shellType = com.paneedah.weaponlib.render.shells.ShellParticleSimulator.Shell.Type.ASSAULT;
			PlayerWeaponInstance pwi = ClientModContext.getContext() != null ? ClientModContext.getContext().getMainHeldWeapon() : null;
			if (pwi != null && pwi.getWeapon() != null) {
				shellType = pwi.getWeapon().getShellType();
			}

			net.minecraft.entity.Entity renderViewEntity = MC.getRenderViewEntity();
			double interpX = renderViewEntity != null ? renderViewEntity.lastTickPosX + (renderViewEntity.posX - renderViewEntity.lastTickPosX) * event.getPartialTicks() : p.posX;
			double interpY = renderViewEntity != null ? renderViewEntity.lastTickPosY + (renderViewEntity.posY - renderViewEntity.lastTickPosY) * event.getPartialTicks() : p.posY;
			double interpZ = renderViewEntity != null ? renderViewEntity.lastTickPosZ + (renderViewEntity.posZ - renderViewEntity.lastTickPosZ) * event.getPartialTicks() : p.posZ;

			GlStateManager.pushMatrix();
			GlStateManager.translate(spawnPos.x - interpX, spawnPos.y - interpY, spawnPos.z - interpZ);
			GlStateManager.rotate(-p.rotationYaw, 0, 1, 0);
			GlStateManager.rotate(p.rotationPitch, 1, 0, 0);
			GlStateManager.scale(1.2, 1.2, 1.2);

			GlStateManager.enableLighting();
			GlStateManager.enableTexture2D();
			MC.getTextureManager().bindTexture(com.paneedah.weaponlib.compatibility.shells.ShellRegistry.getShellTexture(shellType));
			if (com.paneedah.weaponlib.compatibility.shells.ShellRegistry.getShellModel(shellType) != null) {
				com.paneedah.weaponlib.compatibility.shells.ShellRegistry.getShellModel(shellType).render();
			}
		...
		if (ClientModContext.getContext() != null && ClientModContext.getContext().getMainHeldWeapon() != null) {
			...
			if (pwi.getState() == WeaponState.READY) {
				...
				// Only clear the empty-variant flag once a magazine has actually been
				// inserted (ammo > 0). While the weapon is READY but empty (magazine
				// removed, waiting for insert), keep the flag so the upcoming LOAD
				// animation correctly plays the empty/slide-back variant.
				if (pwi.getAmmo() > 0) {
