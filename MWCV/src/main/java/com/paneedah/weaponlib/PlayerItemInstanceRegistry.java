...
public class PlayerItemInstanceRegistry {
	...
	private Map<UUID, Map<Integer, PlayerItemInstance<?>>> clientRegistry = new HashMap<>();
	private Map<UUID, Map<Integer, PlayerItemInstance<?>>> serverRegistry = new HashMap<>();

	private Map<UUID, Map<Integer, PlayerItemInstance<?>>> getRegistry(EntityLivingBase player) {
	...
	public <T extends PlayerItemInstance<S>, S extends ManagedState<S>> T getMainHandItemInstance(EntityLivingBase player, Class<T> targetClass) {
		...
		if (player.world != null && player.world.isRemote && WeaponRenderer.isLoweringActive && WeaponRenderer.getLoweringItemStack() != null) {
			PlayerItemInstance<?> loweringInstance = getItemInstance(player, WeaponRenderer.getLoweringItemStack());
			if (targetClass.isInstance(loweringInstance)) {
				return targetClass.cast(loweringInstance);
			}
		}
	...
	public PlayerItemInstance<?> getMainHandItemInstance(EntityLivingBase player) {
		...
		if (player.world != null && player.world.isRemote && WeaponRenderer.isLoweringActive && WeaponRenderer.getLoweringItemStack() != null) {
			PlayerItemInstance<?> loweringInstance = getItemInstance(player, WeaponRenderer.getLoweringItemStack());
			if (loweringInstance != null) {
				return loweringInstance;
			}
		}
	...
	public PlayerItemInstance<?> getItemInstance(EntityPlayer player, int slot) {
		Map<UUID, Map<Integer, PlayerItemInstance<?>>> reg = getRegistry(player);
		Map<Integer, PlayerItemInstance<?>> slotInstances = reg.computeIfAbsent(player.getPersistentID(), p -> new HashMap<>());
	...
	public <S extends ManagedState<S>, T extends PlayerItemInstance<S>> boolean update(S newManagedState, T extendedStateToMerge) {
		...
		EntityLivingBase player = extendedStateToMerge.getPlayer();
		Map<Integer, PlayerItemInstance<?>> slotContexts = getRegistry(player).get(player != null ? player.getPersistentID() : extendedStateToMerge.getPlayer().getUniqueID());
	...
	public PlayerItemInstance<?> getItemInstance(EntityLivingBase player, ItemStack itemStack) {
		...
		try {
			result = itemStackInstanceCache.get(itemStack, () -> {
				...
				if(instance != null && instance.getPlayer() == null && player != null) {
					instance.setPlayer(player);
				}
	...
	public void update(EntityPlayer player) {
		...
		Map<Integer, PlayerItemInstance<?>> slotContexts = getRegistry(player).get(player.getPersistentID());
		if(slotContexts != null) {
