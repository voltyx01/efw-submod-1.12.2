...
public class AnimationModeProcessor {
	...
	private final ScreenBounds capturedBounds = new ScreenBounds();
	private static final FloatBuffer modelviewBuf = BufferUtils.createFloatBuffer(16);
	private static final FloatBuffer projectionBuf = BufferUtils.createFloatBuffer(16);
	private static final IntBuffer viewportBuf = BufferUtils.createIntBuffer(16);
	private static final FloatBuffer winPosBuf = BufferUtils.createFloatBuffer(3);

	public void resetCapturedBounds() {
		capturedBounds.valid = false;
		capturedBounds.minX = Float.MAX_VALUE;
		capturedBounds.minY = Float.MAX_VALUE;
		capturedBounds.maxX = -Float.MAX_VALUE;
		capturedBounds.maxY = -Float.MAX_VALUE;
		capturedBounds.centerX = 0.0f;
		capturedBounds.centerY = 0.0f;
		capturedBounds.width = 0.0f;
		capturedBounds.height = 0.0f;
	}

	public ScreenBounds getCapturedBounds() {
		return capturedBounds;
	}

	public void captureAttachment(ModelBase model) {
		modelviewBuf.rewind();
		...
		// 1. Проецируем опорную точку начала координат детали (0, 0, 0)
		projectPoint(0.0f, 0.0f, 0.0f, scaleFactor, displayHeight);

		// 2. Проецируем вершины кубов модели, если они доступны
		if (model != null && model.boxList != null && !model.boxList.isEmpty()) {
			for (ModelRenderer mr : model.boxList) {
				if (mr == null || mr.cubeList == null) continue;
				float rx = mr.rotationPointX;
				float ry = mr.rotationPointY;
				float rz = mr.rotationPointZ;
				for (ModelBox box : mr.cubeList) {
					if (box == null) continue;
					projectPoint(rx + box.posX1, ry + box.posY1, rz + box.posZ1, scaleFactor, displayHeight);
					projectPoint(rx + box.posX2, ry + box.posY2, rz + box.posZ2, scaleFactor, displayHeight);
					projectPoint(rx + box.posX1, ry + box.posY1, rz + box.posZ2, scaleFactor, displayHeight);
					projectPoint(rx + box.posX2, ry + box.posY1, rz + box.posZ1, scaleFactor, displayHeight);
					projectPoint(rx + box.posX1, ry + box.posY2, rz + box.posZ1, scaleFactor, displayHeight);
					projectPoint(rx + box.posX2, ry + box.posY2, rz + box.posZ2, scaleFactor, displayHeight);
				}
			}
		}

		if (capturedBounds.minX != Float.MAX_VALUE && capturedBounds.maxX != -Float.MAX_VALUE) {
			capturedBounds.width = Math.max(1.0f, capturedBounds.maxX - capturedBounds.minX);
			capturedBounds.height = Math.max(1.0f, capturedBounds.maxY - capturedBounds.minY);
			capturedBounds.centerX = (capturedBounds.minX + capturedBounds.maxX) / 2.0f;
			capturedBounds.centerY = (capturedBounds.minY + capturedBounds.maxY) / 2.0f;
			capturedBounds.valid = true;
		}
	}

	private void projectPoint(float x, float y, float z, int scaleFactor, int displayHeight) {
		winPosBuf.rewind();
		modelviewBuf.rewind();
		projectionBuf.rewind();
		viewportBuf.rewind();
		if (GLU.gluProject(x, y, z, modelviewBuf, projectionBuf, viewportBuf, winPosBuf)) {
			float sx = winPosBuf.get(0) / (float) scaleFactor;
			float sy = (displayHeight - winPosBuf.get(1)) / (float) scaleFactor;
			capturedBounds.minX = Math.min(capturedBounds.minX, sx);
			capturedBounds.maxX = Math.max(capturedBounds.maxX, sx);
			capturedBounds.minY = Math.min(capturedBounds.minY, sy);
			capturedBounds.maxY = Math.max(capturedBounds.maxY, sy);
		}
	}
