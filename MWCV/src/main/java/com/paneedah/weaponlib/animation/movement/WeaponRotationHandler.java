...
public class WeaponRotationHandler {
	...
	public void run(RenderContext<RenderableState> renderContext, StateDescriptor stateDescriptor) {
		...
		float aimProgress = renderContext.getTransitionProgress();
		...
		boolean fromZoom = (fromState == RenderableState.ZOOMING || fromState == RenderableState.ZOOMING_SHOOTING || fromState == RenderableState.ZOOMING_RECOILED);
		boolean toZoom = (toState == RenderableState.ZOOMING || toState == RenderableState.ZOOMING_SHOOTING || toState == RenderableState.ZOOMING_RECOILED);

		float aimFactor = 0f;
		if (fromZoom && toZoom) {
			aimFactor = 1.0f;
		} else if (toZoom) {
			aimFactor = aimProgress;
		} else if (fromZoom) {
			aimFactor = 1.0f - aimProgress;
		} else if (renderContext.getWeaponInstance().isAimed()) {
			aimFactor = 1.0f;
		}
		...
		if (aimFactor > 0f) {
			...
			if (renderContext.getWeaponInstance().getScope() != null
				...
				swayAmplitude = swayAmplitude + (SWAY_AMPLITUDE_OPTICAL_ADS - swayAmplitude) * aimFactor;
				walkingSwayAmplitude *= (1.0f - aimFactor * 0.67f);
			}
			...
			// it look better.
			recoverySpring.setDamping(5.0f * aimFactor + 1.0f * (1.0f - aimFactor));

			float forwardScale = 1.0f + (FORWARD_MAGNITUDE_ADS_DIVISOR * divisorMultiplier - 1.0f) * aimFactor;
			float strafeScale = 1.0f + (STRAFE_MAGNITUDE_ADS_DIVISOR * divisorMultiplier - 1.0f) * aimFactor;
			float recoilScale = 1.0f + (RECOIL_AMPLITUDE_ADS_DIVISOR * divisorMultiplier - 1.0f) * aimFactor;
			float recoveryScale = 1.0f + (WEAPON_RECOVERY_ADS_DIVISOR * divisorMultiplier - 1.0f) * aimFactor;
			float walkingSwayScale = 1.0f + (WALKING_SWAY_ADS_DIVISOR * divisorMultiplier - 1.0f) * aimFactor;

			forwardMagnitude *= forwardScale;
			strafeMagnitude *= strafeScale;
			recoilAmplitude *= recoilScale;
			weaponRecoveryAmplitude *= recoveryScale;
			walkingSwayAmplitude *= walkingSwayScale;
		}
