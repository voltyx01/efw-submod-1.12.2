...
public class AnimationData {
	...
	public static float ANIMATION_SPEED_MULTIPLIER = 1.35f;
	public static final float PACE = 833f / ANIMATION_SPEED_MULTIPLIER;
	...
	public List<Transition<RenderContext<RenderableState>>> getTransitionListDual(Transform normal, Transform ads, double divisor) {
		...
		if(!isNull) {
			int count = 0;
			for (Entry<Float, BlockbenchTransition> bb : this.bbTransition.entrySet()) {
				Transition<RenderContext<RenderableState>> transition = (Transition<RenderContext<RenderableState>>) bb.getValue().createVMWTransitionWithADS(normal, ads, divisor);
				if(count == 0) transition.setDuration(200);
		...
		curLength = Math.max(curLength, 150);
	...
	public List<Transition<RenderContext<RenderableState>>> getTransitionList(Transform initial, double divisor, boolean applySwap) {
		...
		if(applySwap) {
			...
			long curLength = transitionList.get(transitionList.size()-1).getDuration();
			if(curLength < 150) {
				transitionList.get(transitionList.size()-1).setDuration(150);
			}
		}
