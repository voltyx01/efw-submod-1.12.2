...
public class MultipartRenderStateManager<State, Part, Context extends PartPositionProvider> {
	...
	private class TransitionedPositioning implements MultipartPositioning<Part, Context> {
			...
			try {
				return partDataMap.computeIfAbsent(part, p -> {
					...
					if(fromMatrix == null && fromPositioning != null) {
                        if (fromState == RenderableState.NORMAL && toState == RenderableState.DRAWING) {
                            fromMatrix = getMatrixForPositioning(toPositioning.get(0), p, context);
                            fromMatrix.m31 += 1.5f; // Start further down to create upward movement instead of freezing (Y points down)
                        } else if (fromState == RenderableState.ZOOMING && ("MAIN_ITEM".equals(p.toString()) || "RIGHT_HAND".equals(p.toString()))) {
                            fromMatrix = lastApplied.get(p);
                            if (fromMatrix == null && fromPositioning != null && !fromPositioning.isEmpty()) {
                                fromMatrix = getMatrixForPositioning(fromPositioning.get(fromPositioning.size() - 1), p, context);
                            }
                        } else if ((p instanceof com.paneedah.weaponlib.ItemMagazine || p.getClass().getSimpleName().toLowerCase().contains("mag")) && ("RELOADING".equals(String.valueOf(toState)) || "LOAD_EMPTY".equals(String.valueOf(toState)))) {
                            fromMatrix = getMatrixForPositioning(toPositioning.get(0), p, context);
                        } else {
		...
		public Positioner<Part, Context> getPositioner() {
			...
			if(previousIndex != currentIndex) {
				// Only play sounds while no GUI screen (inventory / container) is open.
				// If thirdPersonOnly, additionally require the player to be in third-person
				// view so we don't duplicate sounds with the first-person state machine.
				boolean canPlaySound = targetState.sound != null
						&& MC.currentScreen == null
						&& (!thirdPersonOnly || MC.gameSettings.thirdPersonView != 0);
						
				// Suppress any sounds from the animation JSON if we are drawing, because
				// WeaponRenderer already played pistol_draw or other_draw.
				if (toState != null && toState.toString().equals("DRAWING")) {
					canPlaySound = false;
				}
			...
			com.paneedah.weaponlib.stats.AttachmentStatsManager.EffectiveWeaponStats effStats = null;
			if (MC.player != null) {
				PlayerWeaponInstance pwi = com.paneedah.weaponlib.ClientModContext.getContext() != null
						? com.paneedah.weaponlib.ClientModContext.getContext().getMainHeldWeapon() : null;
				if (pwi != null) {
					effStats = com.paneedah.weaponlib.stats.AttachmentStatsManager.getEffectiveStats(pwi);
				}
			}

			if (toState != null) {
				String st = toState.toString();
				if ("DRAWING".equals(st)) {
					speedMultiplier = com.paneedah.weaponlib.config.ModernConfigManager.drawSpeedMultiplier;
					if (effStats != null) {
						speedMultiplier *= effStats.drawSpeedMultiplier;
					}
				} else if ("UNEQUIPPING".equals(st)) {
					speedMultiplier = com.paneedah.weaponlib.config.ModernConfigManager.unequipSpeedMultiplier;
					if (effStats != null) {
						speedMultiplier *= effStats.drawSpeedMultiplier;
					}
				} else if ("ZOOMING".equals(st)) {
					if (effStats != null) {
						speedMultiplier *= effStats.adsSpeedMultiplier;
					}
				} else if ("UNLOADING".equals(st) || "RELOADING".equals(st)
						|| "COMPOUND_RELOAD".equals(st) || "COMPOUND_RELOAD_EMPTY".equals(st)
						|| "TACTICAL_RELOAD".equals(st) || "LOAD_ITERATION".equals(st)
						|| "LOAD_ITERATION_COMPLETED".equals(st)
						|| "ALL_LOAD_ITERATIONS_COMPLETED".equals(st)
						|| "LOAD_EMPTY".equals(st) || "UNLOAD_EMPTY".equals(st)
						|| "COMPOUND_RELOAD_ADS".equals(st)) {
					speedMultiplier = com.paneedah.weaponlib.config.ModernConfigManager.reloadSpeedMultiplier;
					if (effStats != null) {
						speedMultiplier *= effStats.reloadSpeedMultiplier;
					}
				}
			}
			if (speedMultiplier <= 0.01) speedMultiplier = 1.0;

			long currentDuration = (long) (targetState.getDuration() / speedMultiplier);
			long currentPause = (long) (targetState.getPause() / speedMultiplier);
			...
			if(currentIndex == 0 && startTime == null) {
				...
				if (toState != null && toState.toString().equals("DRAWING")) {
					long cutoff = com.paneedah.weaponlib.config.ModernConfigManager.drawAnimationCutoffMs;
					currentStartTime = currentTime;
					while (cutoff > 0 && currentIndex < toPositioning.size()) {
						MultipartTransition<Part, Context> t = toPositioning.get(currentIndex);
						long tDur = (long) ((t.getDuration() + t.getPause()) / speedMultiplier);
						if (cutoff >= tDur) {
							cutoff -= tDur;
							currentIndex++;
							currentStartTime = currentTime;
						} else {
							currentStartTime -= cutoff;
							cutoff = 0;
						}
					}
					if (currentIndex < toPositioning.size()) {
						targetState = toPositioning.get(currentIndex);
						currentDuration = (long) (targetState.getDuration() / speedMultiplier);
						currentPause = (long) (targetState.getPause() / speedMultiplier);
					}
				}
			}
			...
			if(currentStartTime == 0) {
				...
				if(LOG.isDebugEnabled() && currentIndex < toPositioning.size()) {
					...
					LOG.debug("Starting transition {}, duration {}ms, pause {}ms", currentIndex,
							(long) (multipartTransition.getDuration() / speedMultiplier), (long) (multipartTransition.getPause() / speedMultiplier));
				}
				currentStartTime = currentTime;
			}
			...
			final MultipartTransition<Part, Context> finalTargetState = targetState;
				...
				return new Positioner<Part, Context>() {
					...
					public void position(Part part, Context context) {
						...
						if(MC.gameSettings.thirdPersonView == 0 && part.toString().contains("MAIN_ITEM")
							...
							if(finalTargetState != null && finalTargetState.beizer != null) {
								beizer = revertFlag ? finalTargetState.beizer.scale(1.25) : finalTargetState.beizer;
							}
			...
			return new Positioner<Part, Context> () {
				...
				public void position(Part part, Context context) {
					...
					/*
					 * try {
						if(finalTargetState.sound != null) {
							MC.getSoundHandler().playSound(PositionedSoundRecord.getMasterRecord(finalTargetState.sound, 1.0f));
					...
					boolean cancelBeizer = true; // Disabled Bezier curve arc during ADS transition
	...
	/**
	 * When true this state manager only plays sounds while in actual third-person
	 * view (thirdPersonView != 0).  Set to true for the third-person / GUI manager
	 * so it doesn't duplicate sounds that the first-person state machine already plays.
	 */
	private boolean thirdPersonOnly = false;

	public void setThirdPersonOnly(boolean thirdPersonOnly) {
		this.thirdPersonOnly = thirdPersonOnly;
	}
