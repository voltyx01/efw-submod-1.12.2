...
//        private Map<RenderableState, ScreenShaking> screenShakings = new HashMap<>();
        ...
        private int[][] guiPositions = new int[][] {{-43, 86},{97, 96},{92, 34},{14, -55},{164, -26},{145, 67},{60, 110},{0, -50},{-100, -50},{-29, 44},{50, 100},{0, 100}};
        ...
        public Weapon build(ModContext modContext) {
            ...
            ItemMagazine defaultMag = null;
            for(CompatibleAttachment<Weapon> attachment : compatibleAttachments.values()) {
                if(attachment.getAttachment() instanceof ItemMagazine) {
                    ItemMagazine mag = (ItemMagazine) attachment.getAttachment();
                    if(defaultMag == null || mag.getCapacity() < defaultMag.getCapacity()) {
                        defaultMag = mag;
                    }
                }
            }
            if(defaultMag != null) {
                CompatibleAttachment<Weapon> ca = compatibleAttachments.get(defaultMag);
                if (ca != null) {
                    ca.setDefault(true);
                }
            }
    ...
    public void toggleAiming() {
        ...
        if (mainHandHeldWeaponInstance == null) return;

        WeaponState state = mainHandHeldWeaponInstance.getState();
        boolean isCurrentlyAimed = mainHandHeldWeaponInstance.isAimed();

        // Always allow un-aiming (even during reload) so holdToAim/toggle key release works
        boolean isReloadState = state == WeaponState.LOAD
                || state == WeaponState.LOAD_REQUESTED
                || state == WeaponState.LOAD_ITERATION
                || state == WeaponState.LOAD_ITERATION_COMPLETED
                || state == WeaponState.ALL_LOAD_ITERATIONS_COMPLETED
                || state == WeaponState.AWAIT_FURTHER_LOAD_INSTRUCTIONS
                || state == WeaponState.UNLOAD
                || state == WeaponState.UNLOAD_PREPARING
                || state == WeaponState.UNLOAD_REQUESTED
                ...
                || state == WeaponState.COMPOUND_EMTPY_REQUESTED;

        if (isCurrentlyAimed && isReloadState) {
            // Only allow un-aim during reload, not re-aim
            mainHandHeldWeaponInstance.setAimed(false);
            return;
        }

        if (state == WeaponState.READY
                || state == WeaponState.PAUSED
                || (state == WeaponState.COMPOUND_RELOAD_FINISHED && mainHandHeldWeaponInstance.wasReloadedFromEmpty)
                || state == WeaponState.EJECT_REQUIRED
                || state == WeaponState.DRAWING) {
            mainHandHeldWeaponInstance.setAimed(!isCurrentlyAimed);
        }
    }
    ...
    String getCrosshair(PlayerWeaponInstance weaponInstance) {
        if(weaponInstance.isAimed()) {
            ...
            return crosshair;
        } else if(weaponInstance.getPlayer() != null && weaponInstance.getPlayer().isSprinting()){
...
//            ClientEventHandler.BULLET_HOLE_RENDERER.addBulletHole(new BulletHole(new Vector3D(position.hitVec.x, position.hitVec.y, position.hitVec.z), position.sideHit, 0.05));
    ...
    public PlayerWeaponInstance createItemInstance(EntityLivingBase player, ItemStack itemStack, int slot){
        ...
        if (attachmentIds != null && attachmentIds.length > 0) {
            instance.setActiveAttachmentIds(attachmentIds);
        } else {
