...
public class PlayerItemInstance<S extends ManagedState<S>> extends UniversalObject implements ExtendedState<S> {
    ...
    public EntityLivingBase getPlayer() { return player; }
    public void setPlayer(EntityLivingBase player) { this.player = player; }
    public Item getItem() { return item; }
    public int getItemInventoryIndex() { return itemInventoryIndex; }
    public void setItemInventoryIndex(int itemInventoryIndex) { this.itemInventoryIndex = itemInventoryIndex; }
    public long getSyncStartTimestamp() { return syncStartTimestamp; }
...
//	private Set<PlayerItemStateListener<S>> listeners = new HashSet<>();
    ...
    public ItemStack getItemStack() {
        if (player instanceof EntityPlayer && itemInventoryIndex >= 0 && itemInventoryIndex < ((EntityPlayer) player).inventory.getSizeInventory()) {
            ItemStack stack = ((EntityPlayer) player).inventory.getStackInSlot(itemInventoryIndex);
            if (stack != null && !stack.isEmpty()) {
                return stack;
            }
        }
        return this.itemStack != null ? this.itemStack : ItemStack.EMPTY;
    }
