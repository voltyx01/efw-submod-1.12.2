...
public class PostProcessPipeline {
	...
	public static void setWorldElements() {
		...
		if (isOriginalWeatherRendererDirty || (currentWeatherRenderer != modernWeatherRenderer
				&& currentWeatherRenderer != originalWeatherRenderer)) {
			originalWeatherRenderer = currentWeatherRenderer;
			isOriginalWeatherRendererDirty = false;
		}

		if (ModernConfigManager.enableFancyRainAndSnow && originalWeatherRenderer == null) {
			MC.world.provider.setWeatherRenderer(modernWeatherRenderer);
			MC.effectRenderer.registerParticle(EnumParticleTypes.WATER_DROP.getParticleID(),
					new ParticleFancyRain.Factory());
		} else if (originalWeatherRenderer != null) {
	...
	public static void recreateDepthFramebuffer() {
		 ...
		 * 
		 * if(normalDepthTexture == null) {
		 * normalDepthTexture = new DepthTexture(buffer.framebufferWidth,
		 * buffer.framebufferHeight);
		 * }
		 * 
		 * // Method automatically tests if it should so there
		 * // is very little overhead here.
		 * normalDepthTexture.recreateBuffer(buffer.framebufferWidth,
		 * buffer.framebufferHeight);
		 * 
		 ...
		 * GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GLCompatible.GL_DEPTH_COMPONENT24,
		 * width, height, 0,
		 ...
		 * log.
		 * error("Depth framebuffer creation returned an incomplete attachment error.");
		 ...
		 * log.
		 * error("Depth framebuffer creation returned an incomplete draw buffer error."
		 * );
		 * } else if (status == OpenGlHelper.GL_FB_INCOMPLETE_READ_BUFFER) {
		 * log.
		 * error("Depth framebuffer creation returned an incomplete read buffer error."
		 * );
		 * } else {
	...
	public static void blitDepth() {
		 ...
		 * depthBuffer);
		 * 
		 * GLCompatible.glBlitFramebuffer(0, 0, width, height, 0, 0, width, height,
	...
	public static void drawRainBuffer() {
		...
		// System.out.println("There are " + (rainKeepAlive ? "still" : "no") + " drops
		// left.");
