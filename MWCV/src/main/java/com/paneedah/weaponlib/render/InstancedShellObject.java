...
public class InstancedShellObject extends ModelInstancedObject<ShellManager> {
	...
	public int updateData(ArrayList<Shell> shells) {
		...
		*/
		...
		for(Shell sh : shells) {
			...
			{
				double dx = sh.pos.x - MC.player.posX;
				double dy = sh.pos.y - MC.player.posY;
				double dz = sh.pos.z - MC.player.posZ;
				if (dx*dx + dy*dy + dz*dz > 100.0) continue;
			}
			actualShells++;
